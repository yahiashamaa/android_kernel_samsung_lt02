        ...
        struct device_attribute *attr, const char *buf, size_t size)
{
	...
	dev_info(&client->dev, "%s,data = %d\n",__func__,data);

	cap->dummy_threshold = data;
	data -= cap->key_threshold;

	if (write_reg(info->client,
			BT532_DUMMY_BUTTON_SENSITIVITY, data) != I2C_SUCCESS) {
			dev_info(&client->dev, "failed to set touchkey data %d.\n",
				data);
			return -1;
	}
	return data;
...
retry:
	...
	if (val > 1200) {
		dev_info(&client->dev, "%s: Invalid %d's key sensitivity = %d\n",
			__func__, i, val);
		goto retry;
	}
...
static DEVICE_ATTR(touchkey_threshold, S_IRUGO | S_IWUSR | S_IWGRP,
					show_touchkey_threshold, store_touchkey_dummy_threshold);

...
static int pm860x_audio_suspend(struct snd_soc_codec *codec)
{
	struct pm860x_priv *pm860x_audio = snd_soc_codec_get_drvdata(codec);
	int status1, status2;
	...
	if (((status2 & (1 << 0)) == 0) && ((status1 & (1 << 4)) == 0)) {
		 ...
		 * need to observe whether will get pop and click, and should
		 * write audio regitster before audio section is turned off
		 */
...
static int pm860x_audio_resume(struct snd_soc_codec *codec)
{
	 ...
	 * suspend since we are in a voice call
	 */
	...
	if ((status1 & (1 << 5)) && (status1 & (1 << 3)) &&
		...
		/*
		 * The audio I2C registers (register addresses 0x0 to 0xEE)
		 * are * essible for read or write activity 160 usec after
		 * AUDIO_PU set to '1'
		 */
		msleep(1);
...
static irqreturn_t pm860x_short_handler(int irq, void *data)
{
	...
	int offset = PM860X_AUDIO_BASE - PM860X_AUDIO_OFFSET;

	/* check audio SHORTS status */
	hs_status = pm860x_reg_read(pm860x->i2c, PM860X_SHORTS + offset);
	pr_debug("%s: Headset short status 0x%x\n", __func__, hs_status);
	/* check whether HS1 SHORT status is set */
	if (hs_status & 0x02) {
		/* clear HS1 status */
		pm860x_set_bits(pm860x->i2c, PM860X_SHORTS + offset, 0x02,
				0x02);
		/* check DAC reg */
		dac_status =
		    pm860x_reg_read(pm860x->i2c, PM860X_DAC_EN_1 + offset);
		if (!(dac_status & 0x1))
			/* HS1 amplifier has been disabled, enable it */
			pm860x_set_bits(pm860x->i2c, PM860X_DAC_EN_1 + offset,
					0x1, 1);
	}
	...
	if (hs_status & 0x08) {
		/* clear HS2 status */
		pm860x_set_bits(pm860x->i2c, PM860X_SHORTS + offset, 0x08,
				0x08);
		/* check DAC reg */
		dac_status =
		    pm860x_reg_read(pm860x->i2c, PM860X_DAC_EN_1 + offset);
...
static int pm860x_probe(struct snd_soc_codec *codec)
{
	...
	snd_soc_add_codec_controls(codec, pm860x_audio_controls,
			     ARRAY_SIZE(pm860x_audio_controls));
	snd_soc_add_codec_controls(codec, pm860x_snd_controls,
...
static int pm860x_remove(struct snd_soc_codec *codec)
{
	...
	for (i = IRQ_NUM; i > 0; i--)
		free_irq(pm860x->irq[i - 1], pm860x);
...
static int __devinit pm860x_codec_probe(struct platform_device *pdev)
{
	...
	if (!res) {
		dev_err(&pdev->dev, "Failed to get short IRQ resources\n");
		goto out;
	}
	pm860x->irq[0] = res->start + chip->irq_base;
	strncpy(pm860x->name[0], res->name, MAX_NAME_LEN);

	ret =
	    request_threaded_irq(pm860x->irq[0], NULL, pm860x_short_handler,
				 IRQF_ONESHOT, "audio short", pm860x);
	if (ret < 0) {
	...
	if (ret) {
		...
		goto register_codec_fail;
	}
	return ret;

register_codec_fail:
	free_irq(pm860x->irq[0], pm860x);
out:
	platform_set_drvdata(pdev, NULL);
	devm_kfree(&pdev->dev, pm860x);
	return -EINVAL;
}

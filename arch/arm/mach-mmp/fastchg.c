 ...
 *
 *   0 - disabled (default)
 *   1 - use custom mA configured through sysfs interface (see below)
 *
 ...
 *
 *   0 - disabled - allow anything up to 2.3A to be used as AC / USB custom current
...
static ssize_t force_fast_charge_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	switch (force_fast_charge) {
		...
		case FAST_CHARGE_FORCE_CUSTOM_MA:	return sprintf(buf, "1 - Use custom mA on AC (%dmA) and USB (%dmA)\n",ac_charge_level,usb_charge_level);
...
static ssize_t ac_charge_level_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	if (failsafe == FAIL_SAFE_DISABLED && new_ac_charge_level <= MAX_CHARGE_LEVEL) {
		...
		switch (new_ac_charge_level) {
			...
			case AC_CHARGE_2100:
			case AC_CHARGE_2200:
			case AC_CHARGE_2300:		ac_charge_level = new_ac_charge_level;
							return count;
			default:			return count;
...
static ssize_t usb_charge_level_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	switch (usb_charge_level) {
		case USB_CHARGE_500:		return sprintf(buf, "[500]  600  700  800  900  1000\n");
		case USB_CHARGE_600:		return sprintf(buf, "500  [600]  700  800  900  1000\n");
		case USB_CHARGE_700:		return sprintf(buf, "500  600  [700]  800  900  1000\n");
		case USB_CHARGE_800:		return sprintf(buf, "500  600  700  [800]  900  1000\n");
		case USB_CHARGE_900:		return sprintf(buf, "500  600  700  800  [900]  1000\n");
		case USB_CHARGE_1000:		return sprintf(buf, "500  600  700  800  900  [1000]\n");
		default:			return sprintf(buf, "Custom : %dmA\n",usb_charge_level);
	}
}
...
{
	return sprintf(buf, "Forced Fast Charge by laufersteppenwolf v1\n");
}
